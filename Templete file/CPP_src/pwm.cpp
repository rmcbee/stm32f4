...
void pwm::set(uint8_t dutyCycle)
{
	...
	{
		timerNum->CCR1 = period * dutyCycle / 255;
	}
	else if(controlRegisterNum == 2)
	{
		timerNum->CCR2 = period * dutyCycle / 255;
	}
	else if(controlRegisterNum == 3)
	{
		timerNum->CCR3 = period * dutyCycle / 255;
	}
	else if(controlRegisterNum == 4)
	{
		timerNum->CCR4 = period * dutyCycle / 255;
	}
}

void pwm::timerFactor(TIM_TypeDef* timer)
{
	
	if(timer == TIM1)
	{
		period *= 2;  //scale the period to match the clock
	}
	else if(timer == TIM2)
	{
		return; //do nothing
...
uint16_t pwm::getPinSource(uint16_t pin)
{
	if(pin == GPIO_Pin_0)
	{
...
void pwm::bankToClock(GPIO_TypeDef* bank)
{
...
void pwm::timerToClock(TIM_TypeDef* timer)
{
